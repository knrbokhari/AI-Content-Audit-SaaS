...
const Navbar = ({ onMenuToggle }: any) => {
  ...
  return (
    <header
      className="sticky top-0 z-40 flex items-center h-14 px-4 lg:px-6 border-b shrink-0"
      style={{ background: "var(--bg-surface)", borderColor: "var(--border)" }}
    >
      {/* Mobile menu button */}
      <button
        onClick={onMenuToggle}
        className="lg:hidden mr-3 p-1.5 rounded-lg hover:bg-[--bg-surface-2]"
      ...
      {/* Breadcrumb */}
      <nav className="flex items-center gap-1 text-xs flex-1 min-w-0"></nav>
      ...
      <div className="flex items-center gap-1 ml-3">
        ...
        <button
          onClick={toggleTheme}
          className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[--bg-surface-2] hover:cursor-pointer transition-colors"
        >
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            className="w-8 h-8 rounded-lg hover:cursor-pointer flex items-center justify-center hover:bg-[--bg-surface-2] transition-colors relative"
          >
            <Bell size={15} />
            {unread > 0 && (
          ...
          {showNotif && (
            <div className="absolute right-0 top-full mt-1 w-80 rounded-xl border shadow-lg surface animate-slide-up z-50">
              ...
              >
                ...
                {unread > 0 && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 font-600">
                    {unread} new
                  </span>
                )}
              </div>
              ...
              >
                {notifs.map((n: any) => (
                  <div
                    key={n.id}
                    className={clsx(
                      "px-4 py-3 cursor-pointer hover:bg-[--bg-surface-2] transition-colors",
        ...
        <div className="relative ml-1" ref={userRef}>
          <button
            ...
            className="flex items-center gap-2 pl-2 pr-2.5 py-1.5 rounded-lg hover:bg-[--bg-surface-2] transition-colors"
          >
